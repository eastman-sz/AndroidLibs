...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	static final float FRICTION = 3.5f;
...
//		SoundPoolUtil.Instance().suspend(SoundType.PULL_DOWN);
	}
	...
	final void setState(State state, final boolean... params) {
		...
		switch (mState) {
			case RESET:
				onReset();
				Log.e("setState", "RESET");
//				SoundPoolUtil.Instance().suspend(SoundType.PULL_DOWN);
//				//完成后播放声音
//				SoundPoolUtil.Instance().playSounds(SoundType.LOOSEN);
				break;
			case PULL_TO_REFRESH:
				Log.e("setState", "PULL_TO_REFRESH");
//				SoundPoolUtil.Instance().playLoopSounds(SoundType.PULL_DOWN);
				onPullToRefresh();
				break;
			case RELEASE_TO_REFRESH:
				Log.e("setState", "RELEASE_TO_REFRESH");
				onReleaseToRefresh();
				break;
			case REFRESHING:
			case MANUAL_REFRESHING:
				Log.e("setState", "REFRESHING");
				onRefreshing(params[0]);
				break;
			case OVERSCROLLING:
				Log.e("setState", "OVERSCROLLING");
				// NO-OP
				break;
		}
	...
	private void pullEvent() {
		...
		//保证最大值一定是大于上下拉显示的View的高度
		int maxSize = dip2px(getContext() , 91);
		maxSize = itemDimension > maxSize  ? (itemDimension + 6) : maxSize;
		
//		//设置最大值（可上下拉范围）
		if (newScrollValue > maxSize) {
			newScrollValue = maxSize;
		}else if (newScrollValue < (-maxSize)) {
			newScrollValue = (-maxSize);
		}
...
//				Log.e("iwylog",  "PULL_TO_REFRESH:  " +itemDimension + "    :    " + Math.abs(newScrollValue));
			} else if (mState == State.PULL_TO_REFRESH && itemDimension < Math.abs(newScrollValue)) {
				setState(State.RELEASE_TO_REFRESH);
//				Log.e("iwylog", "RELEASE_TO_REFRESH:  " +itemDimension + "    :    " + Math.abs(newScrollValue));
			}
		}
	}
